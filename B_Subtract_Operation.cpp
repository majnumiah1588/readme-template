...
#define inp(v,n)                     vector<ll>v(n,0); for(auto &a: v) cin >> a;
...
#define Find(v,x)                    auto it = std::find(all(v), x); ((it != v.end())?Y:N);

void solve()
{
    in2(n,k);  inp(v,n); ijw bool ans=false;
    i=0,j=1;
    if(n==1 and v[0]==k) ans=true;
    else{
        sort(all(v));
         while(i<n and j<n){
        if(v[i]+abs(k)==v[j]) {
