...
{
   FastIO;
   in(n); inp(v,n); sort(all(v)); int c1=0,c2=0,c3=0,c4=0,ans=0;
    fr(i,0,n) {
       if(v[i]==1) c1++; if(v[i]==2) c2++;  if(v[i]==3) c3++; if(v[i]==4) c4++;
   }
    ans+=(c4+c2/2+c1/4); c1=c1%4; c2=c2%2;
    if(c3>=c1) { c3=c3-c1; ans+=c1; c1=0; ans+=(c2+c3); c2=0; c3=0; }
    else if(c1>c3) {
       ans+=c3; c1=c1-c3; c3=0;
       if(c1<=2) {ans+=1; c2=0; c1=0;}
       else if(c1>2){
            ans+=1; c1=c1-2; c2=0; if(c1%4==0) { ans+=c1/4; c1=0;} else { ans+=c1/4; if(c1%4!=0) ans+=1; }
       }
