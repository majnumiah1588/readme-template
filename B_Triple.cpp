...
{
  ll i,j,cn=0,mx=0;
  in(n); inp(v,n); sort(all(v));  ll l=v[0];
  fr(i,0,n){
    if(v[i]==l) { cn++; mx=max(mx,cn); if(mx>=3) {cc<<l<<nl; return ;} }
    else { l=v[i]; cn=1; mx=max(mx,cn); }
  }
  if(mx<3) cc<<"-1"<<nl; else cc<<mx<<nl;
}
int main()
{
   FastIO;
