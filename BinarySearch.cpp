...
#define mx(v)                        std::max_element(all(v))
#define mn(v)                        std::min_element(all(v))
#define uniq(v)                      unique(all(v))
#define uniqsz(v)                    uniq(v)-(v).begin()
#define mxIndx(v)                    mx(v)-(v).begin()
#define mnIndx(v,mnv)                std::distance((v).begin(),mnv)
...
void solve()
{
 ...
 while(l<=r){
  m=(l+r)/2;
  if(v[m]>v[m-1] && v[m]>v[m+1]) { cc<<m<<" "<<v[m]<<endl; return; }
  (v[m-1]<v[m] && v[m+1]>v[m])? l=m+1 : r=m-1 ;
 }
 cc<<-1<<nl;
//cc<<((l>r)?-1:v[m])<<nl;
