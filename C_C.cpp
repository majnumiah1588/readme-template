...
#define print(v)                     for(auto a: v) cerr << a<< " "; cerr<<nl;
...
void solve(){
   ...
   fr(i,0,sz(s))  m[s[i]-'0']++;
   for(char i='A'; i<='Z'; i++){
      if(m[(ll)i/10]>0 && m[(ll)i%10]>0 && ((ll)i/10!=(ll)i%10)||((ll)i/10==(ll)i%10 && m[(ll)i/10]>1 && m[(ll)i%10]>1 )) cc<<i; else continue;
    }cc<<nl;
}
int main()
{
