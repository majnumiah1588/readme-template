...
void solve()
{
   ...
   ll ck,p,k;
   if(n>=3){
       ss+=s[n-3]; ss+=s[n-2]; ss+=s[n-1];
       ck=stoll(ss);
    if(ck%8==0) { cc<<s<<nl; return; }
    else {
        ss[2]='0'; ck=stoll(ss);
        if(ck%8==0) { s[n-3]=ss[0]; s[n-2]=ss[1]; s[n-1]=ss[2]; cc<<s<<nl; return; }
        ss[2]='2'; ck=stoll(ss);
        if(ck%8==0) { s[n-3]=ss[0]; s[n-2]=ss[1]; s[n-1]=ss[2]; cc<<s<<nl; return; }
        ss[2]='4'; ck=stoll(ss);
        if(ck%8==0) { s[n-3]=ss[0]; s[n-2]=ss[1]; s[n-1]=ss[2]; cc<<s<<nl; return; }
   ...
   if(p%8==0) { cc<<s<<nl; return; }
   else{
    s[1]='0';  k=stoll(s); if(k%8==0) { cc<<s<<nl; return; }
    s[1]='2';  k=stoll(s); if(k%8==0) { cc<<s<<nl; return; }
    s[1]='4';  k=stoll(s); if(k%8==0) { cc<<s<<nl; return; }
    s[1]='6';  k=stoll(s); if(k%8==0) { cc<<s<<nl; return; }
    s[1]='8';  k=stoll(s); if(k%8==0) { cc<<s<<nl; return; }
   }
}
int main()
{
