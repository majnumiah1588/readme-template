...
void solve()
{
    ...
    fr(i,0,n-1)
    {
    if(v[i]==zz) { cnt++; mx=max(mx,cnt); }
    else{
         zz=v[i]; cnt=1; mx=max(mx,cnt);
        }
    }
    ...
    if(mx>=2) cc<<N;
        ...
        cc<<Y;
        for(i=0;i<n;i++) {
            if(v[i]&1)  v1.pb(v[i])  ;    //cc<<v[i]<<" ";
        }
                for(i=0;i<n;i++) {
                    if(v[i]%2==0)    v1.pb(v[i] ) ;   //cc<<v[i]<<" ";

                }

                        for(i=n-1;i>=0;i--) {
                            if(v[i]&1) v2.pb(v[i] ) ; //cc<<v[i]<<" ";

                        }
                        for(i=n-1;i>=0;i--) {
                            if(v[i]%2==0) v2.pb(v[i] ) ;        // cc<<v[i]<<" ";
                            }
