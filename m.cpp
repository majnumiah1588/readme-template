...
void solve()
{
    in3(n,m,k); inp(jj,n); inp(gg,m);  ll ans=0,z=0,s2=0,s1=0,s3=0;
    sort(all(jj)); rsort(gg);  bool f=true;  fr(i,0,n) z+=jj[i];
    if(jj[0]>=gg[0]) f=false;
    
    if(k&1 && f==true) {
         z=z-jj[0]+gg[0];
          OP(z); return ;
         }
         else if(f==true && k%2==0){
            z=z-jj[0]+min(gg[m-1],jj[0]); OP(z); return;
         }
    else if(f==false){
        if(k&1) { ans=z; OP(z); return;}
        else { jj[n-1]=gg[m-1]; fr(i,0,n) s2+=jj[i];  OP(s2); return;  }
    }
}
int main()
{
