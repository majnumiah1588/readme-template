...
#define lowerBound(v,l)              lower_bound(all(v),l)-(v).begin();
#define Case                         cout<<"Case "<<++ii<<": "<<ans<<endl;
#define FastIO                       ios_base::sync_with_stdio(0);cin.tie(0);cout.tie(0);

int p1,p2;
bool solve( const std::vector<int>&v, int k, int n )
{
   int l=0,r=n-1,sum;  // l=left..... r=right .... k=target value
   while(l<r){
     sum=v[l]+v[r];
     if(sum==k) { p1=v[l],p2=v[r]; return true; } // (p1, p2) = pair where p1+p2=k
     (sum<k) ? l++ : r-- ;
   }
    return false;
}
...
{
   FastIO;  // TC
   in(n); inp(v,n); sort(all(v)); in(k);
   bool ans=solve(v,k,n);
   (ans) ? (cc<<Y<<p1<<" "<<p2<<nl) : (cc<<N) ;
   // DT
}
